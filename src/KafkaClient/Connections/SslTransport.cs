                    ...
                    async () => {
                        ...
                        _tcpSocket = await _configuration.ConnectionRetry.TryAsync(
                            ...
                            async (attempt, elapsed) => {
                                ...
                                _log.Verbose(() => LogEvent.Create($"Attempting SSL connection to {_endpoint}, SslProtocol:{_sslConfiguration.EnabledProtocols}, Policy:{_sslConfiguration.EncryptionPolicy}"));
