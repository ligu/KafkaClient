...
namespace KafkaClient.Tests.Unit
{
    ...
    public class EndpointTests
    {
        ...
        {
            var endpoint1 = await Endpoint.ResolveAsync(new Uri("http://localhost:8888"), TestConfig.Log);
            var endpoint2 = await Endpoint.ResolveAsync(new Uri("http://localhost:8888"), TestConfig.Log);
        ...
        {
            var endpoint1 = await Endpoint.ResolveAsync(new Uri("http://localhost:8888"), TestConfig.Log);
            var endpoint2 = await Endpoint.ResolveAsync(new Uri("http://localhost:1"), TestConfig.Log);
